...
export class MainComponent implements OnInit, AfterViewInit {
  ...
  setTimestampAuto() {
    ...
    const parseDerepoDate = (dateString: string): Date | null => {
      ...
      return new Date(2018, parseInt(matches[1], 10) - 1, parseInt(matches[2], 10), parseInt(matches[3], 10), parseInt(matches[4], 10));
    };
    ...
    if (index === 0) {
      const nextTalkDate = parseDerepoDate(this.setting.talkList[index + 1].date);
      if (nextTalkDate === null) {
        return;
      }
      this.nowTalk.date = formatDate(subRandomMinute(nextTalkDate), 'MM-dd HH:mm');
      return;
    }
    if (index === this.setting.talkList.length - 1) {
      const prevTalkDate = parseDerepoDate(this.setting.talkList[index - 1].date);
      if (prevTalkDate === null) {
        return;
      }
      this.nowTalk.date = formatDate(addRandomMinute(prevTalkDate), 'MM-dd HH:mm');
      return;
    }
