...
const DerepoView: React.FC<{ talkData: ITalkData; firstFlg?: boolean }> = ({
  talkData,
  firstFlg = false
}) => {
  // 外側のdivの参照
  ...
  React.useEffect(() => {
    ...
    // メッセージ欄の下端を取得
    const messageBottom =
      messageRef.current.offsetTop + messageRef.current.offsetHeight;
    ...
    setFavsCss({
      fontSize: calc(12),
      left: calc(355 - (talkData.favs.length * 8) / 2),
      top: calc(33)
    });
    if (firstFlg) {
      setDatetimeCss({
        fontSize: calc(12),
        left: calc(305),
        top: Math.max(calc(55), messageBottom - 5)
      });
    } else {
      setDatetimeCss({
        fontSize: calc(12),
  ...
  return (
    <div ref={overallRef} className="position-relative" style={overallCss}>
      ...
      />
      <span className="position-absolute name" style={nameCss}>
        {talkData.name}
      </span>
      <span
        ref={messageRef}
        className="position-absolute message"
        style={messageCss}
      >
        {talkData.message}
      </span>
      <span className="position-absolute star-base" style={starBaseCss} />
      <span className="position-absolute star" style={starCss}>
        ★
      </span>
      <span className="position-absolute favs" style={favsCss}>
        {talkData.favs}
      </span>
      <span className="position-absolute datetime" style={datetimeCss}>
        {talkData.datetime}
      </span>
    </div>
  );
