...
const PreviewTalkForm: React.FC<{ className?: string }> = ({
  className = ''
}) => {
  ...
  return (
    <div className={className}>
      ...
      {config.talkType === 'おはなし'
        ? OhanashiPreviewTalkForm(config)
        : DerepoPreviewTalkForm(config)}
      <Form>
        <Button className="w-100 my-3" variant="primary" onClick={saveImage}>
          画像として保存(※)
        </Button>
      </Form>
      <p>
        <strong>
          ※……Webブラウザの仕様により、iOSでは画像保存ボタンが効かないことがあります
        </strong>
      </p>
    </div>
  );
